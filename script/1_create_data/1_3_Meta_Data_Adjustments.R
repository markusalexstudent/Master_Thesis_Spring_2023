...
ret <- ret %>% 
  ...
  mutate(
    ...
    lead_retMkt2 = dplyr::lead(retMkt, 2),
    lead_retMkt3 = dplyr::lead(retMkt, 3),
    lead_retMkt4 = dplyr::lead(retMkt, 4),
    lead_retMkt5 = dplyr::lead(retMkt, 5),
    
    lag_retMkt = dplyr::lag(retMkt),
    lag_retMkt2 = dplyr::lag(retMkt, 2),
    lag_retMkt3 = dplyr::lag(retMkt, 3),
    lag_retMkt4 = dplyr::lag(retMkt, 4),
    lag_retMkt5 = dplyr::lag(retMkt, 5),
    
    # Event window returns (FF3):  
    ret_contempMkt = (1+lag_retMkt)*(1+retMkt)*(1+lead_retMkt)-1) %>% 
  select(date, ticker, retMkt, lead_retMkt, lag_retMkt, ret_contempMkt,
         lead_retMkt2, lead_retMkt3, lead_retMkt4, lead_retMkt5, 
         lag_retMkt2, lag_retMkt3, lag_retMkt4, lag_retMkt5)
...
# Check tha tthe NAs are only in the figure: 
sum(!complete.cases(meta$lag_retMkt2), !complete.cases(meta$lag_retMkt3),
    !complete.cases(meta$lag_retMkt4), !complete.cases(meta$lag_retMkt5),
    !complete.cases(meta$lead_retMkt2),!complete.cases(meta$lead_retMkt3),
    !complete.cases(meta$lead_retMkt4), !complete.cases(meta$lead_retMkt5))

#saveRDS(meta, file = "fromR/metaAdj_v4.rds")
